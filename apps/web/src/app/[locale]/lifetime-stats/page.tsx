...
export async function generateMetadata({ params }: LifetimeStatsPageProps): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "lifetimePage" });
  return {
    title: t("metaTitle"),
    description: t("metaDescription"),
    alternates: buildAlternates("/lifetime-stats", locale),
  };
}

export default async function LifetimeStatsPage({ params }: LifetimeStatsPageProps) {
  const { locale } = await params;
  ...
  if (!data) {
    redirect({ href: "/login?next=/lifetime-stats", locale });
    return null;
  }
