...
import { CharacterSummary } from "../../domain/entities/anime.entity";
import { AnimeNotFoundException } from "../../domain/exceptions/anime.exceptions";
import { ANIME_REPOSITORY } from "../tokens";

@Injectable()
export class GetAnimeCharactersUseCase implements UseCase<string, CharacterSummary[]> {
  constructor(
    @Inject(ANIME_REPOSITORY)
    private readonly animeRepository: AnimeRepositoryPort,
  ) {}

  async execute(slug: string): Promise<CharacterSummary[]> {
