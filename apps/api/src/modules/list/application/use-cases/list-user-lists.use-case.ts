...
interface Input {
  ...
  /** Only applied to the "public" filter. Default "popular". */
  sort?: ListSort;
}
...
export class ListUserListsUseCase implements UseCase<Input, ListSummary[]> {
  ...
  async execute({ userId, filter, sort }: Input): Promise<ListSummary[]> {
    if (filter === "mine") return this.repo.findByUserId(userId);
    if (filter === "liked") return this.repo.findLikedByUserId(userId);
    return this.repo.findPublic(PUBLIC_LIMIT, sort ?? "popular");
  }
}
